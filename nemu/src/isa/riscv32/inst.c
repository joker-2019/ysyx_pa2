...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(rd) = (uint32_t)src1 / (uint32_t)src2); //除法。
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (uint32_t)src1 >> (imm & 0x1F));//算术右移立即数
  ...
  //分支指令
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, s->dnpc = ((int32_t) src1 >= (int32_t)src2) ? s->pc + SEXT(imm, 13): s->pc + 4);//分支指令  大于等于
