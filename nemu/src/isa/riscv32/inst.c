...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  // INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, printf("csrrw instruction: rs1=%x, imm=0x%x\n", src1, imm); word_t t = csr_read(imm); printf("Before csrrw: CSR=0x%x, rs1 value=0x%x\n", t, src1); csr_write(imm, src1); R(rd) = t); // 控制状态寄存器读后写
  // INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, printf("csrrs instruction: rs1=%x, imm=0x%x\n", src1, imm); word_t t = csr_read(imm); printf("Before csrrs: CSR=0x%x, rs1 value=0x%x\n", t, src1); csr_write(imm, t|src1); R(rd) = t); // 控制状态寄存器读后置位
  ...
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, CSRs[MEPC]=s->pc; CSRs[MCAUSE]=14; s->dnpc =CSRs[MTVEC]); // 11 /* Machine ECALL */
