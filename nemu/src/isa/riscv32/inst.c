...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  //分支指令
  //INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, s->dnpc = ((int32_t) src1 >= (int32_t)src2) ? s->pc + SEXT(imm, 13): s->pc + 4);//有符号数比较 分支指令  大于等于
