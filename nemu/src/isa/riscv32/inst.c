...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw, I, word_t t = csr_read(imm); csr_write(imm, src1); if(rd != 0) R(rd) = t); // 控制状态寄存器读后写
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs, I, word_t t = csr_read(imm); csr_write(imm, t | src1); if(rd != 0) R(rd) = t); // 控制状态寄存器读后置位
